...
Function New-SfB-RTCLOCAL
{
    ...
    {
        Write-Host "Installing SQL Express Instance RTCLOCAL" -ForegroundColor Green
        ...
        Start-Process -FilePath "C:\Program Files\Skype for Business Server 2019\Deployment\Bootstrapper.exe" -ArgumentList "/BootstrapLocalMgmt /SourceDirectory:`"$Skype4BusinessPath\Setup\amd64`"" -Wait
    }
    Else 
    {
        Write-Host "SQL Express Instance RTCLOCAL already exists." -ForegroundColor Green
    }
}
...
Function New-SfB-LYNCLOCAL
{
    ...
    {
        Write-Host "Installing SQL Express Instance LYNCLOCAL" -ForegroundColor Green
        Test-FilePath ("C:\Program Files\Skype for Business Server 2019\Deployment\Bootstrapper.exe")
        Test-FilePath ("$Skype4BusinessPath\Setup\amd64")
        Start-Process -FilePath "C:\Program Files\Skype for Business Server 2019\Deployment\Bootstrapper.exe" -ArgumentList "/SourceDirectory:`"$Skype4BusinessPath\Setup\amd64`"" -Wait
    }
    Else 
    {
    ...
    # Rerun Install-CsDatabase in case there were failures from last time
    Install-CsDatabase -ConfiguredDatabases -SqlServerFqdn "$SkypeFQDN" -Report "$env:TEMP\Install-CsDatabase-RTCLOCAL-$DTG-v2.html"
}
...
Function Set-SfB-DNS
{
    ...
    If(!($TestDNS6)) 
    {
        Add-DnsServerResourceRecord -ZoneName $DomainDnsName -ComputerName $DC -Name "_sipinternaltls._tcp" -Srv -DomainName "$SkypeFQDN" -Priority 0 -Weight 0 -Port 5061 -TimeToLive 00:05:00
        #Add-DnsServerResourceRecord -ZoneName $DomainDnsName -ComputerName $DC -Name "_sipinternaltls._tcp" -Srv -DomainName $DomainDnsName -Priority 0 -Weight 0 -Port 506 -TimeToLive 00:05:00
