...
Write-Host -foregroundcolor green "Copying WSUS Script/SCCMShare folders to C:\ and D:\"
...
Copy-Item $MDTSTAGING\WSUSImports -Destination D:\ -Recurse
...
Copy-Item $MDTSTAGING\SCCMShare\* -Destination D:\SCCMShare -Recurse
Copy-Item $MDTSTAGING\WSUSImports\WsusContent\* -Destination E:\WSUS\WsusContent -Recurse
...
C:\WSUSMaint\SQLCMD\VC_redist.x64.exe /passive /norestart
...
$argumentList = @(
  '/i'
  '"{0}"' -f "C:\WSUSMaint\SQLCMD\msodbcsql.msi"
  '/passive'
  '/norestart'
...
$argumentList = @(
  '/i'
  '"{0}"' -f "C:\WSUSMaint\SQLCMD\MsSqlCmdLnUtils.msi"
  '/passive'
  '/norestart'
